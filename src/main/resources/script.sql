...
DROP TABLE IF EXISTS h_publishers;
...
DROP TABLE IF EXISTS h_authors;
...
DROP TABLE IF EXISTS h_book_authors;
...
DROP TABLE IF EXISTS h_book_categories;
...
DROP TABLE IF EXISTS h_book_images;
...
DROP TABLE IF EXISTS h_access_credentials;
...
DROP TABLE IF EXISTS h_user_cards;
...
DROP TABLE IF EXISTS h_user_order;
...
DROP TABLE IF EXISTS h_user_cart;
...
DROP TABLE IF EXISTS h_cart_items;
...
DROP TABLE IF EXISTS h_order_payment;
...
DROP TABLE IF EXISTS h_vouchers;
...
DROP TABLE IF EXISTS h_wishlist;
...
DROP TABLE IF EXISTS h_wishlist_item;
	...
	BEGIN
		...
		ELSE
			INSERT INTO h_user_order SELECT
				...
				coalesce(NEW.ord_last_modified, OLD.ord_last_modified),
...
CREATE OR REPLACE FUNCTION audit_user_cart() RETURNS TRIGGER AS $audit_user_cart$
	BEGIN
		IF (TG_OP = 'INSERT') THEN
			INSERT INTO h_user_cart SELECT NEW.*, 'INSERT';
		ELSE
	...
	BEGIN
		...
		ELSE
			INSERT INTO h_vouchers SELECT
				...
				coalesce(NEW.vou_amount, OLD.vou_amount),
